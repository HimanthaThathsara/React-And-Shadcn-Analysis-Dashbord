import {Activity, Battery, Clock, Gauge, Zap, TrendingUp} from "lucide-react";
...
import {powerConsumptionData, backupPowerStatus, generatorFuelLevel, powerDowntime, runtimeDistribution} from "@/lib/mock-data";

const radialChartData = [{name: "Fuel Level", level: 78, fill: "var(--color-safari)"}];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <StatCard title="Power Consumption" value={`${latestPowerConsumption}V`} icon={<Zap />} description="Current voltage" trend={{value: 2.5, isPositive: true}} />
        <Card>
        ...
        <Card>
          ...
          <CardContent className="flex items-center justify-between">
            ...
            <StatusIndicator status={generatorFuelLevel > 30 ? "normal" : "warning"} label={generatorFuelLevel > 30 ? "Normal" : "Low"} />
          </CardContent>
        </Card>
        <StatCard title="Power Downtime" value={`${powerDowntime}h`} icon={<Activity />} description="Last 24 hours" />
      </div>
          ...
          <CardContent className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{top: 10, right: 30, left: 0, bottom: 0}}>
                ...
                <XAxis dataKey="time" tick={{fontSize: 12}} tickFormatter={(value) => value.split(":")[0]} interval={Math.floor(chartData.length / 10)} />
                <YAxis domain={[180, 250]} tick={{fontSize: 12}} tickFormatter={(value) => `${value}V`} />
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  labelLine={false}>
                  {runtimeDistribution.map((_, index) => (
        ...
        <Card className="col-span-1">
          ...
          <CardContent className="flex-1 pb-0">
            <ChartContainer config={chartConfig} className="mx-auto aspect-square max-h-[250px]">
              <RadialBarChart data={radialChartData} startAngle={90} endAngle={378} innerRadius={80} outerRadius={110}>
                <PolarGrid
                  gridType="circle"
                  radialLines={false}
                  stroke="none"
          ...
          <CardFooter className="flex-col gap-2 text-sm">
            ...
            <div className="leading-none text-muted-foreground">Showing total fuel level in the generator</div>
          </CardFooter>
        </Card>
